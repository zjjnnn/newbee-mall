...
public class CategoryServiceImpl implements CategoryService {
	...
	public List<CategoryListVo> findCategoryById(Integer categoryById) {
		// 取出所有的内容
		List<TbNewbeeMallGoodsCategory> entityList = egCategoryMapper.selectGoodsDetail(categoryById);
		// 创建一个需要返回的vo展示集合对象
		List<CategoryListVo> voList = new ArrayList<>();
		//所有内容进行遍历
		for (TbNewbeeMallGoodsCategory entity : entityList) {
	...
	public List<CategoryListVo> getChildrens(CategoryListVo root, List<TbNewbeeMallGoodsCategory> entityList) {
		//由于子分类下可能还有子分类，因此依旧需要创建一个需要返回的vo展示集合对象
	    List<CategoryListVo> voList = new ArrayList<>();   
	    //依旧将所有内容进行遍历
	    for (TbNewbeeMallGoodsCategory entity : entityList) {
	    	//取出相应子类，加到集合中并返回
	    	if (root.getCategoryId().equals(entity.getParentId())) {
				CategoryListVo vo = new CategoryListVo();
				BeanUtils.copyProperties(entity,vo);
				//使用递归，循序渐进式设置完各大小级分类
				vo.setSubList(getChildrens(vo,entityList));
